...
void Handle_Ray_Sensor(dGeomID o1, dGeomID o2) {
    ...
    if ( dGeomGetClass(o1) == dRayClass ) {
        ...
        if ( n>0 ) {
            ...
            obj->Set_Ray_Sensor(contact.geom.depth,contact.geom.pos[0],contact.geom.pos[1],contact.geom.pos[2],obj2,timer);
        }
    }
}
...
static void simLoop (int pause)
{
	...
	for ( int i = 0; i < int(data->speed) ; i ++ ) 
	...
	environment->Draw(timer-1,data->evaluationTime,data->debug);
...
int main (int argc, char **argv)
{
    ...
    if ( data->runBlind )
      ...
      // dsSimulationLoop (argc,argv,int(352*1.5),int(288*1.5),&fn);
      // dsSimulationLoop (argc,argv,352*3,288*3,&fn);
      dsSimulationLoop (argc,argv,int(1020*1.15),580,&fn);
