...
void OBJECT::Draw(int t, int evalPeriod) {

	if ( raySensor )

		raySensor->Draw(t);

	if ( fade_strategy == NO_FADE ) {

                if ( touchSensor && touchSensor->Is_Firing(t) )

                        dsSetColor(r/3,g/3,b/3);
                else
                        dsSetColor(r,g,b);
	}
	else if ( fade_strategy == FADE_IN )

		dsSetColorAlpha( r, g, b, float(t)/float(evalPeriod) );

	else // fade out

		dsSetColorAlpha( r, g, b, 1 - float(t)/float(evalPeriod) );

    const dReal *pos = dBodyGetPosition(body);

    dReal newPos[3];

    newPos[0] = pos[0] + draw_offset_x;
    newPos[1] = pos[1] + draw_offset_y;
    newPos[2] = pos[2] + draw_offset_z;
    ...
    if (myShape == BOX){
        ...
        dsDrawBox (newPos,rot,sides);
    }
    else if (myShape == CYLINDER)
        dsDrawCylinder(newPos,rot,length,radius);
    else if (myShape == CAPSULE)
        dsDrawCapsule(newPos,rot,length,radius);
    else if (myShape == SPHERE)
        dsDrawSphere(newPos,rot,radius);
}
...
void OBJECT::Read_From_Python(dWorldID world, dSpaceID space, int shape) {
    ...
    std::cin >> fade_strategy;

    CreateBody(world, space);

}

void OBJECT::Set_Ray_Sensor(double distance, double endX, double endY, double endZ, OBJECT *objectThatWasHit, int t) {
	if ( raySensor )
		raySensor->Set(distance,endX,endY,endZ,objectThatWasHit,t);
}
